...
class App extends React.Component {
    ...
    async fetchRootNodes() {
        const root = await fetchData('https://2jdg5klzl0.execute-api.us-west-1.amazonaws.com/default/EmployeesChart-Api?manager=0');
        if (root[0]) {
            const children = await fetchData(`https://2jdg5klzl0.execute-api.us-west-1.amazonaws.com/default/EmployeesChart-Api?manager=${root[0].id}`);
            this.setState({
                data: [...root, ...children],
                isLoading: false
            });
    ...
    getNodeChildren = async (node) => {
        ...
        const children = await fetchData(`https://2jdg5klzl0.execute-api.us-west-1.amazonaws.com/default/EmployeesChart-Api?manager=${nodeId}`);
        this.setState({ isFetchingMoreNodes: false });
        return children;
    };
    handleZoomIn = () => {
        this.setState({
            initialZoom: this.state.initialZoom + 0.1,
        });
    };
    handleZoomOut = () => {
        this.setState({
                  ...
                  <div className="col">
                      <div ref={this.root} className="app">
                          {!this.state.isLoading &&
                              <ButtonsPanel onZoomOut={this.handleZoomOut} onZoomIn={this.handleZoomIn} />}
                          {this.state.isLoading ?
                              <Loading />
                              :
                              <Svg
                              ...
                              />}
                            {this.state.isFetchingMoreNodes &&
                                <FullScreenLoader />}
                      </div>
                  </div>
              </div>
